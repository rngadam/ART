...
public class ClientActivity extends Activity {
	...
	private Runnable updaterRunnable = new Runnable() {
		...
		public void run() {
			...
			try {
				if(!isPaused) {
					updaterHandler.postDelayed(updaterRunnable, 40);
				}
			} finally {
				lock.unlock();
			}
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		toggleButtonForwardLeft = getToggleButton(R.id.toggleButtonForwardLeft, RobotCommands.Directions.FORWARD_LEFT);
		toggleButtonForward = getToggleButton(R.id.toggleButtonForward, RobotCommands.Directions.FORWARD);
		toggleButtonForwardRight = getToggleButton(R.id.toggleButtonForwardRight, RobotCommands.Directions.FORWARD_RIGHT);
		toggleButtonLeft = getToggleButton(R.id.toggleButtonLeftSide,  RobotCommands.Directions.NEUTRAL);
		toggleButtonNeutral = getToggleButton(R.id.toggleButtonNeutral,  RobotCommands.Directions.NEUTRAL);
		toggleButtonRight = getToggleButton(R.id.toggleButtonRightSide,  RobotCommands.Directions.NEUTRAL);
		toggleButtonBackwardLeft = getToggleButton(R.id.toggleButtonBackwardLeft, RobotCommands.Directions.REVERSE_LEFT);
		toggleButtonBackward = getToggleButton(R.id.toggleButtonBackward,  RobotCommands.Directions.REVERSE);
		toggleButtonBackwardRight = getToggleButton(R.id.toggleButtonBackwardRight, RobotCommands.Directions.REVERSE_RIGHT);
		tableLayout.setShrinkAllColumns(true);
		tableLayout.setStretchAllColumns(true);
    }
    
	private void bindReceiverService() {
		Toast.makeText(this, "Connecting to service " + ReceiverService.class.getName(),
				Toast.LENGTH_SHORT).show();
		// Bind to the service
		Intent intent = getReceiverServiceIntent();
		
		bindService(intent,
				receiverServiceConnection, Context.BIND_AUTO_CREATE);
	}   
	...
	private ToggleButton getToggleButton(int id, final RobotCommands.Directions direction) {
		...
		toggleButton.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton buttonView,
					boolean isChecked) {
				if(isChecked) {
					robotCommands.direction = direction;
	...
	private void processRobotUpdate() {
		update(robotData.collisions, toggleButtonForward, RobotData.FORWARD_INFRARED);
		update(robotData.collisions, toggleButtonForwardLeft, RobotData.FORWARD_LEFT_INFRARED);
		update(robotData.collisions, toggleButtonForwardRight, RobotData.FORWARD_RIGHT_INFRARED);
		update(robotData.collisions, toggleButtonLeft, RobotData.LEFT_INFRARED);
		update(robotData.collisions, toggleButtonRight, RobotData.RIGHT_INFRARED);
		update(robotData.collisions, toggleButtonBackward, RobotData.BACKWARD_INFRARED);
	}
	...
	private Intent getReceiverServiceIntent() {
		...
		intent.putExtra(ReceiverService.SERVER_ADDRESS_ATTRIBUTE, preferences.getString(ClientConstants.SERVER_IP_PREF, ClientConstants.DEFAULT_IP));
		intent.putExtra(ReceiverService.SERVER_PORT_ATTRIBUTE, preferences.getInt(ClientConstants.SERVER_PORT_PREF, ClientConstants.DEFAULT_PORT));
		return intent;
	}
